...
import { RoomSheetStatus } from "./RoomSheetStatus";
...
export function RoomItem({ room }) {
  ...
  return (
    <>
      ...
      />
      <RoomSheetStatus
        visible={roomSheetStatus}
        closeOverlay={() => {
          if (room.status) {
            setRoomSheetStatus(false);
          }
        }}
        actions={actions}
        roomInfo={room}
      ...
      onLongPress={() => { room.status == "Occupied"? Toast.show('Room is Currently Occupied') :
      setRoomSheetStatus(true);
    }}
    onPress={() => {
