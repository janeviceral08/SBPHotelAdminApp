...
export function Report_Result_Nationality({ navigation, route}) {
  
    const {checkout, getResult, getResultGoods, Res_Ref, res_out,Res_In,FD_Ref, FD_out, FD_in, ewallet, Debit,Credit,Cash,Cancel_Res,All_Ref  } = useTasks();
 ...
 const searchData = (text) => {
  ...
  const newData = checkout.filter(item => {
    const itemData = item.nationality.toUpperCase();
  ...
  useEffect(() => {
      ...
      var data = [];
    for(var issue of  checkout){
        var entryFound = false;
        var tempObj = {
          name: issue.nationality,
          count: 1
        };
  ...
  return (
   ...
   <View style={styles.mainContainer}>
          ...
          <Item>
            ...
            <Input placeholder="Search Nationalities" style={{borderColor: 'red'}}    onChangeText={(text) => searchData(text)}/>
        ...
        <Tabs renderTabBar={()=> <ScrollableTab tabsContainerStyle={{backgroundColor: Colors.BackColor }}/>}>
          ...
          {Nationality && Nationality.length > 0 ?
                <FlatList
                    ...
                    onPress={() => {
                     navigation.navigate('Report_Details_Nationality', {
                            idNote: item.name
                        })
                    }}
                   
                >
                    <Text style={styles.textTitle} numberOfLines={1}>Nationality:  {item.name}</Text>
                    <Text style={styles.textTime} numberOfLines={1}> {item.count} Guest</Text>
