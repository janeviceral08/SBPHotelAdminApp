...
export function Report_Result({navigation, route}) {
  
    const {checkout, getResult, getResultGoods, Res_Ref, res_out,Res_In,FD_Ref, FD_out, FD_in, ewallet, Debit,Credit,Cash,Cancel_Res,All_Ref,getResultin, changed_room  } = useTasks();
 ...
 const searchData = (text) => {
  ...
  const result = () => {
    ...
    if(category == 'All Check-in')
    {
      ...
      navigation.navigate('Report_Result')
    }
  ...
  return (
   ...
   <View style={styles.mainContainer}>
          ...
          <Item>
            ...
            <Input placeholder="Checked-in Report Search Customer Name" style={{borderColor: 'red'}}    onChangeText={(text) => searchData(text)}/>
                ...
                <FlatList
                    ...
                    <CardItem bordered button onPress={()=> navigation.navigate('Customer_Details', {checkinInfo: item})}>
                      <Left  style={{marginTop: -10, marginBottom: -10}}>
                        ...
                        <Body>
                          <Text>{item.customer}</Text>
                          <Text note style={{width: '150%'}}>{currencyFormat(parseFloat(item.overall_total))} ({item.number_of_days != ''? item.number_of_days + ' Nights': item.number_of_hours + 'hours'})</Text>
                        </Body>
                      </Left>
                      <Right>
                      <MaterialCommunityIcons name={'dots-vertical'} size={18} color={'black'} style={{ paddingTop: 5}} onPress={()=> navigation.navigate('Customer_Details', {checkinInfo: item})}/>
