...
export function ManageAdminTeam({ navigation, route }) {
  ...
  const addTeamMembers = async () => {
    try {
      setLoading(true)
      await user.functions.addTeamMember(newTeamMember, userData.expiration, "Admin");
      setLoading(false)
      getTeam();
    } catch (err) {
      setLoading(false)
  ...
  <View style={styles.manageTeamWrapper}>
    ...
    <Button onPress={() => addTeamMembers(newTeamMember)} title="Add Admin" color={Colors.buttons}/>
  </View>
