...
export function Report_Details_Nationality({ navigation, route}) {
  
    const {checkout, getResult, Res_Ref, res_out,Res_In,FD_Ref, FD_out, FD_in, ewallet, Debit,Credit,Cash,Cancel_Res,All_Ref  } = useTasks();
...
const natl = [...new Map(checkout.map(item => [item[key_nat], item])).values()]
  ...
  const requestRunTimePermission=()=>{
    ...
    const name = 'Nationality ('+ date_now +' )'+'.xlsx'
  ...
  return (
   ...
   <View style={styles.mainContainer}>
          ...
          <Item>
            ...
            <Input placeholder="Nationality Report Search Customer Name" style={{borderColor: 'red'}}    onChangeText={(text) => searchData(text)}/>
...
{customers && customers.length > 0 ?
                <FlatList
                    ...
                    renderItem={({item})  =>  
                    route.params.idNote == item.nationality?
          ...
          >
            ...
            body={
              ...
              <Body>
                ...
                {customers && customers.length ? customers.reduce((sum, i) => (   route.params.idNote == i.nationality?
              
                                      sum += parseFloat(i.overall_total):
                                      sum += 0
                                    ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (route.params.idNote == i.nationality?
...
<Card style={{flex: 0, marginTop: 10}} >
  <ScrollView>
              ...
              sum += parseFloat(i.payment_method=='Cash' || i.payment_method == "" && route.params.idNote == i.nationality? i.overall_total: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (

              sum += parseFloat(i.payment_method=='Cash' && route.params.idNote == i.nationality? i.overall_total: 0)
              ...
              sum += parseFloat(i.payment_method=='Debit Card' && route.params.idNote == i.nationality? i.overall_total: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (
              ...
              sum += parseFloat(i.payment_method=='Credit Card' && route.params.idNote == i.nationality? i.overall_total: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (
              ...
              sum += parseFloat(i.payment_method=='E-Wallet' && route.params.idNote == i.nationality? i.overall_total: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (
              ...
              sum += parseFloat(i.checkin_stat== 'Over The Counter' || i.checkin_stat==""|| i.checkin_stat==null && route.params.idNote == i.nationality?i.overall_total:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (

              sum += parseFloat(i.checkin_stat== 'Over The Counter' && route.params.idNote == i.nationality?i.overall_total:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text>  
                                                     </Body>
                                                      </CardItem>
...
<CardItem bordered style={{marginTop: -10}}>
              ...
              sum += parseFloat(i.checkin_stat== 'Reservation' && route.params.idNote == i.nationality?i.overall_total:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : checkout.reduce((sum, i) => (
              ...
              sum += parseFloat(i.checkin_stat== 'Reservation' && route.params.idNote == i.nationality?i.overall_total:0)
                                                      ...
                                                      <CardItem key={item._id} style={{height: 35}}>
                                                        ...
                                                        <Body>
                                                    <Text style={{fontSize: 13}}>{  customers && customers.length ? customers.reduce((sum, i) => (
              sum += parseFloat(i.nationality == item.nationality && route.params.idNote == i.nationality ? 1: 0)
