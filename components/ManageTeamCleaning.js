...
export function ManageTeamCleaning({ navigation, route }) {
  ...
  const addTeamMembers = async () => {
    try {
      setLoading(true)
      await user.functions.addTeamMember(newTeamMember, userData.expiration, "Maintenance");
      setLoading(false)
      getTeam();
    } catch (err) {
      setLoading(false)
  ...
  <View style={styles.manageTeamWrapper}>
    <View style={styles.manageTeamTitle}>
      <Text h3>Maintenance Staffs</Text>
    </View>
    {teamMemberList.map((member) => (
      <ManageTeamCleaningItem  key={`${member._id}`} room={member} />
    ))}

    <Text h4> Add Maintenance Staff:</Text>
    <View style={styles.inputContainer}>
      <TextInput
        onChangeText={(text) => setNewTeamMember(text)}
        value={newTeamMember}
        placeholder="New Maintenance Staff Email"
        style={styles.addTeamMemberInput}
        keyboardType={'email-address'}
      />
    </View>
    <Button onPress={() => addTeamMembers(newTeamMember)} title="Add Maintenance Staff" color={Colors.buttons}/>
